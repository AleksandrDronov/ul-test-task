...
export const CHECKBOX_FILTER_FIELDSETS: CheckboxFilterFieldsetConfig[] = [
  {
    ...
    options: Object.entries(AUCTION_STATUS_LABEL_BY_CODE)
      .map(([code, label]) => ({ value: Number(code), label: AUCTION_STATUS_RU_LABEL[label] }))
  ...
  },
  {
    ...
    options: TRADING_STATUS_VALUES.filter((value): value is TradingStatus => value !== 'Unknown').map(
      (value) => ({ value, label: TRADING_STATUS_RU_LABEL[value] }),
    ),
  },
  {
    key: 'auc_type',
    legend: 'Тип аукциона',
    idPrefix: 'auc-type',
    options: AUC_TYPE_VALUES.map((value) => ({ value, label: AUCTION_TYPE_RU_LABEL[value] })),
  },
]
